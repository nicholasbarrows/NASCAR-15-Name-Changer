...
* On launch, you will be prompted to select a file named ARCHIVE0.AR. If a valid NASCAR '15 installation is detected, the file dialog will open directly in the data folder, where you can pick the file from there.
* Otherwise, navigate to the `data` directory in your NASCAR '15 installation and then choose the file.
* After selecting the ARCHIVE0.AR file, there will be a list of all 46 playable drivers in the game, a textbox next to them, and a number on the far right. The number represents the amount of space you have left in your input
* Leaving the input blank will retain the driver's original name
* Due to the way the game stores strings containing the driver names, by default, the chosen custom names are limited to being less than or equal to to the length of the original driver's name. However, there is the option to go beyond this limit and overwrite data in certain cases.
...
Some drivers have known issues if you disable Safe Mode. None of these issues are game-breaking, but are worth mentioning. I put them in order from most to least noticeable. This list will be updated if more issues are found.
...
| Jeb Burton      | 10           | 31                       | 24                        | Going beyond 24 will overwrite the team name for Go Green Racing, which is shown in the race results and season standings pages                                                                                                   |
...
| Justin Allgaier | 15           | 31                       | 28                        | Going beyond 28 will overwrite a string containing the word "Engine". I haven't confirmed if this string is actually even used anywhere, so this one might not even be an issue at all.  |

#### No Known Issues

Here's a full list of the increased character limits if you disable Safe Mode. The only known issue with any of these is it can sometimes mess with the track record stats on the loading screen, but that's so minor I didn't feel the need to include it as an actual issue. The reason you'll see a lot of 31's on this table is because the max length of a string is 31, the game will truncate strings longer than 31 characters. Not every driver has enough excess data after them to get all the way to 31 characters though. I also included the data that is overwritten when using the full extra space for those curious. Some of these strings are interesting, as there are several names that are not only not in this game, but are not in _any_ Eutechnyx game (some haven't even been in any official NASCAR game at all).
...
| Trevor Bayne      | 12           | 31                       | The data directly after Bayne's name is trivia about his 2009 Nationwide Season (left over from NASCAR 2011)                                            |
...
| Chase Elliott     | 13           | 31                       | The name `Paulie Harraka`, Harraka's hometown `Wayne, NJ`, and Harraka's fictional team from NASCAR '14, `PH LLC`                                                              |
...
| Josh Wise         | 9            | 31                       | Strings containing `Brian Johnson`, `Doug Richert`, and `Kevin Harvick Inc.`. As with Bayne and Danica, pretty clear leftovers from NASCAR 2011 (first game they appeared in) |

## Uninstall

Simply open the ARCHIVE0.AR file in your game directory and click the `Restore Default Names` button. This will restore the original driver name data and overwrites any modifications to the archive.
